            ...
            [
                [
                    bot.types.InlineKeyboardButton(
                        f"Calls {'✅' if q[2] == 'c' and query.message.reply_markup.inline_keyboard[0][0].text.split(' ')[-1] != '✅' else '❎'}", callback_data=f'ses_{q[1]}_c',
                    ), bot.types.InlineKeyboardButton(f"Secret Chats {'✅' if q[2] == 's' and query.message.reply_markup.inline_keyboard[0][1].text.split(' ')[-1] != '✅' else '❎'}", callback_data=f'ses_{q[1]}_s'),
                ], [bot.types.InlineKeyboardButton('Log Out', callback_data=f'logout_{q[1]}')], [bot.types.InlineKeyboardButton('Other Sessions', switch_inline_query_current_chat='getAuths')],
            ],
        ),
    )
